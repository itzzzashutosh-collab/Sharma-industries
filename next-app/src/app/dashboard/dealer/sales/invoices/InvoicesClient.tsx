...
const INVOICE_TEMPLATES = [
  {
    ...
    fields: ["Customer/Contractor", "Brand Products", "CGST/SGST/IGST", "MRP Display"],
  ...
  },
  {
    ...
    fields: ["Customer Details", "Advance Paid", "Balance Due", "Payment Terms"],
    href: "/dashboard/dealer/sales/invoices/new",
  },
  {
    id: "painter-invoice",
    name: "Painter Invoice",
    desc: "Quick invoice for contractors and painters with product/brand selection",
    tag: "Painter",
    tagColor: "bg-purple-500/10 text-purple-600 border-purple-500/20",
    fields: ["Contractor/Painter", "Brand Selection", "MRP & Selling Price", "GST"],
    href: "/dashboard/dealer/sales/invoices/new",
  },
  {
    id: "credit-note",
...
function exportToCSV(invoices: any[]) {
  ...
  link.href = url;
  link.download = `dealer-invoices-${new Date().toISOString().slice(0, 10)}.csv`;
  link.click();
...
function exportToPrint(invoices: any[]) {
  ...
  if (!win) return;
  win.document.write(`<!DOCTYPE html><html><head><title>Invoice Report – Sharma Industries (Dealer)</title>
  <style>
  ...
  <h1>Invoice History – Sharma Industries (Dealer Portal)</h1>
...
export function InvoicesClient({ initialData }: Props) {
  ...
  // Load drafts from localStorage (dealer-specific key)
  const loadDrafts = useCallback(() => {
    try {
      const raw = localStorage.getItem("dealer_invoice_drafts");
  ...
  const deleteDraft = (id: string) => {
    ...
    setDrafts(updated);
    localStorage.setItem("dealer_invoice_drafts", JSON.stringify(updated));
  };
  ...
  const totalDue = invoices.reduce(
    (s, i) => s + (i.balance_due > 0 ? i.balance_due : 0),
    0
  );
      ...
      <div className="flex flex-col gap-4 border-b border-border pb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-foreground tracking-tight">
                {t("GST Invoicing Terminal")}
              </h1>
              <p className="text-xs text-muted-foreground mt-0.5">
                {invoices.length} {t("invoices")} · ₹
                {totalRevenue.toLocaleString("en-IN")} {t("total revenue")}
              </p>
            </div>
          </div>
            ...
            <AnimatePresence>
              {exportOpen && (
                ...
                >
                  ...
                  <button
                    onClick={() => {
                      exportToPrint(filteredInvoices);
                      setExportOpen(false);
                    }}
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 bg-amber-500/8 border border-amber-500/20 rounded-xl px-4 py-2.5">
            ...
            <span className="text-xs font-bold text-foreground">
              {invoices.length - paidCount} {t("Unpaid")} · ₹
              {totalDue.toLocaleString("en-IN")} {t("due")}
            </span>
          </div>
          {drafts.length > 0 && (
            ...
            {(["history", "drafts", "templates"] as ActiveTab[]).map((tab) => (
              ...
              >
                ...
                {tab === "drafts" && drafts.length > 0 && (
                  <span className="ml-0.5 bg-primary/15 text-primary rounded-full px-1.5 py-0.5 text-[10px] font-black">
                    {drafts.length}
                  </span>
                )}
              </button>
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-muted/50 border-b border-border">
                      <th className="py-4 px-5 text-[10px] font-black text-muted-foreground uppercase tracking-wider">
                        {t("Date")}
                      </th>
                      <th className="py-4 px-5 text-[10px] font-black text-muted-foreground uppercase tracking-wider">
                        {t("Invoice No")}
                      ...
                      <th className="py-4 px-5 text-[10px] font-black text-muted-foreground uppercase tracking-wider text-right">
                        {t("Amount")}
                      </th>
                      <th className="py-4 px-5 text-[10px] font-black text-muted-foreground uppercase tracking-wider text-right">
                        {t("Balance Due")}
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-4 px-5 font-semibold text-sm text-foreground">
                              {getCustomerName(inv) || "Retail Customer"}
                            </td>
                            ...
                            >
                              <Link
                                href={
                                  inv.pdf_url ||
                                  `/dashboard/dealer/sales/invoices/${inv.id}`
                                }
              ...
              {filteredInvoices.length > 0 && !isLoading && (
                <div className="px-5 py-3 bg-muted/20 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
                  <span>
                    {filteredInvoices.length} {t("invoices shown")}
                  </span>
                  <span>
                    {t("Total")}:{" "}
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {t("Invoices you started but haven't submitted yet")}
                </p>
              </div>
              <Link
            ...
            {drafts.length === 0 ? (
              <div className="bg-card border border-dashed border-border rounded-2xl p-16 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1.5 max-w-xs mx-auto">
                  {t(
                    'When you create a new invoice and click "Save Draft", it will appear here.'
                  )}
                </p>
                <Link
                  href="/dashboard/ceo/invoices/new"
                ...
                {drafts.map((draft) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5 truncate">
                      {draft.customer || t("No customer selected")}
                    </p>
                    {draft.amount > 0 && (
        ...
        {activeTab === "templates" && (
          ...
          >
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {t("Start a new invoice from a pre-configured template")}
              </p>
            </div>
            ...
            >
              ...
              <div className="p-6 border-b border-border flex items-center justify-between shrink-0">
                <div>
                  ...
                  <h3 className="text-lg font-black text-foreground mt-2">
                    {getCustomerName(selectedInvoice) || "Retail Customer"}
                  </h3>
                  <p className="text-xs text-muted-foreground mt-0.5">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-5">
                  ...
                  {[
                    {
                      ...
                      value:
                        (selectedInvoice.balance_due ?? 0) <= 0
                          ? "Paid"
                          : "Unpaid",
                      isPaid: (selectedInvoice.balance_due ?? 0) <= 0,
                    },
                    {
                      label: t("Payment Mode"),
                      value:
                        selectedInvoice.payment_mode ||
                        selectedInvoice.payment_terms?.mode ||
                        "—",
                    },
                    {
                      label: t("Due Date"),
                    ...
                    {
                      label: t("Client Type"),
                      value: selectedInvoice.client_type || "Customer",
                    },
                  ].map((item, i) => (
                    <div
                      key={i}
                    ...
                    >
                      ...
                      {"isPaid" in item ? (
                        <span
                          className={`inline-block mt-1 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase border ${
                            item.isPaid
                              ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20"
                              : "bg-amber-500/10 text-amber-600 border-amber-500/20"
                          }`}
                ...
                <div className="bg-muted/30 border border-border/50 rounded-2xl p-5">
                  <h4 className="font-black text-[10px] text-muted-foreground uppercase tracking-wider mb-3">
                    {t("Billing Items")}
                  </h4>
                      ...
                      selectedInvoice.items.map((it: any, idx: number) => (
                        ...
                        >
                          ...
                          <span className="font-mono font-black text-foreground">
                            ₹
                            {(
                              (it.qty || it.quantity || 1) * (it.rate || 0)
                            ).toLocaleString("en-IN")}
                          </span>
                        </div>
                      ))
                ...
                <div className="bg-muted/30 border border-border/50 rounded-2xl p-5 space-y-2.5 text-xs">
                  <h4 className="font-black text-[10px] text-muted-foreground uppercase tracking-wider mb-1">
                    {t("Pricing Summary")}
                  </h4>
                  ...
                  {selectedInvoice.cgst > 0 && (
                    <div className="flex justify-between text-muted-foreground">
                      ...
                      <span className="font-mono">
                        ₹
                        {(selectedInvoice.cgst || 0).toLocaleString("en-IN")}
                      </span>
                    </div>
                  )}
                  {selectedInvoice.sgst > 0 && (
                    <div className="flex justify-between text-muted-foreground">
                      ...
                      <span className="font-mono">
                        ₹
                        {(selectedInvoice.sgst || 0).toLocaleString("en-IN")}
                      </span>
                    </div>
                  )}
                  {selectedInvoice.igst > 0 && (
                    <div className="flex justify-between text-muted-foreground">
                      ...
                      <span className="font-mono">
                        ₹
                        {(selectedInvoice.igst || 0).toLocaleString("en-IN")}
                      </span>
                    </div>
                  )}
                  {selectedInvoice.total_gst > 0 &&
                    ...
                    !selectedInvoice.igst && (
                      <div className="flex justify-between text-muted-foreground">
                        ...
                        <span className="font-mono">
                          ₹
                          {(selectedInvoice.total_gst || 0).toLocaleString(
                            "en-IN"
                          )}
                        </span>
                      </div>
                  ...
                  <div className="flex justify-between text-foreground font-black border-t border-border pt-2.5 text-sm">
                    ...
                    <span className="font-mono text-primary">
                      ₹
                      {(selectedInvoice.grand_total || 0).toLocaleString(
                        "en-IN"
                      )}
                    </span>
                  </div>
              ...
              <div className="p-5 border-t border-border bg-muted/20 flex gap-3 shrink-0">
                <Link
                  href={`/dashboard/dealer/sales/invoices/${selectedInvoice.id}`}
