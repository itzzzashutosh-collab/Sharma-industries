...
import { markDealerInvoiceAsPaid } from "./actions";
import { X, Landmark, Check } from "lucide-react";

export function DealerSettlementModal({
  invoice,
  onClose,
  onSuccess,
}: {
  ...
  const [amount, setAmount] = useState(
    (invoice.balance_due || 0).toString()
  );
  ...
  const handleSettle = async () => {
    ...
    if (isNaN(val) || val <= 0 || val > invoice.balance_due) {
      alert("Please enter a valid amount.");
      return;
    }

    setIsSubmitting(true);
    const res = await markDealerInvoiceAsPaid(
      invoice.id,
      val,
      mode,
      notes
      ...
      <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full py-3 mt-2 bg-primary hover:bg-primary/90 text-white font-bold rounded-xl flex justify-center items-center gap-2 disabled:opacity-50"
          >
            {isSubmitting ? (
              "Processing..."
            ) : (
