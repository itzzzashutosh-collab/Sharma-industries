...
export function SalesRegisterClient({ initialData }: { initialData: any[] }) {
  const { t } = useLanguage();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="text-[10px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-1.5 mb-1">
        <span>CA Workspace</span><span className="opacity-40">/</span><span>GST and Tax</span><span className="opacity-40">/</span><span className="text-foreground">Sales Register</span>
      </div>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-primary/10 border border-primary/20 rounded-xl"><FileSpreadsheet size={20} className="text-primary" /></div>
          <div><h1 className="text-xl font-black text-foreground">{t("Sales Register")}</h1><p className="text-xs text-muted-foreground">{filtered.length} entries — Read-only audit view</p></div>
        </div>
