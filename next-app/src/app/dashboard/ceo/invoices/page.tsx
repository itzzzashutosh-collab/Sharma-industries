    ...
    <div className="p-6 max-w-screen-2xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black text-foreground tracking-tight">{t("Invoice History")}</h1>
            <p className="text-muted-foreground mt-1">{t("Manage and track all generated invoices and settlements.")}</p>
          </div>
        </div>
        <Link
        ...
        >
          <Plus size={20} /> {t("Add New Invoice")}
        </Link>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/50 border-b border-border">
                ...
                <th className="py-4 px-6 text-xs font-bold text-muted-foreground uppercase tracking-wider">{t("Invoice No")}</th>
                ...
                <th className="py-4 px-6 text-xs font-bold text-muted-foreground uppercase tracking-wider text-right">{t("Balance Due")}</th>
                <th className="py-4 px-6 text-xs font-bold text-muted-foreground uppercase tracking-wider">{t("Status")}</th>
            ...
            <tbody className="text-sm">
              {isLoading ? (
                <tr>
                  <td colSpan={7} className="py-10 text-center text-muted-foreground font-medium">
                    {t("Loading invoices...")}
                  </td>
                </tr>
                  ...
                  return (
                    <tr key={inv.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="py-4 px-6 text-right font-black text-rose-500">
                        {inv.balance_due > 0 ? `₹${inv.balance_due.toLocaleString()}` : "—"}
                      </td>
                      ...
                      <td className="py-4 px-6 text-right">
                        <Link
                          href={`/dashboard/ceo/invoices/${inv.id}`}
