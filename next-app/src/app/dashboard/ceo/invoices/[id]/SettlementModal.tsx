...
export function SettlementModal({ invoice, onClose, onSuccess }: { invoice: any, onClose: () => void, onSuccess: () => void }) {
  ...
  const handleSettle = async () => {
    ...
    const res = await markInvoiceAsPaid(invoice.id, val, mode, notes);
      ...
      <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
        ...
        <div className="space-y-4">
          <div>
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">Balance Due: ₹{invoice.balance_due}</p>
          </div>
