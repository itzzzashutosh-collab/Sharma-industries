...
export function QuotationEngine() {
  ...
  useEffect(() => {
    if (paymentMode === "Credit" && quotationDate) {
  ...
  }, [paymentMode, quotationDate, creditDays]);

  // Load company settings via APIs
  useEffect(() => {
    setIsMounted(true);
    getNextQuotationNumber().then((no) => setQuotationNo(no));
    setQuotationDate(new Date().toISOString().split("T")[0]);

    const savedTemplate = localStorage.getItem("quotation_template");
    if (savedTemplate) {
  ...
  const handleTemplateChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setTemplate(val);
    localStorage.setItem("quotation_template", val);
  };

  const currentTheme = useMemo(() => {
    return (
      QUOTATION_TEMPLATES.find((t) => t.id === template) || QUOTATION_TEMPLATES[0]
    );
  }, [template]);
      ...
      <div className="flex items-center gap-3 border-b border-border pb-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {t(
              "Create professional tax quotations with zero-storage client-side PDF generation.",
            )}
          </p>
        </div>
      </div>
            ...
            <div className="bg-card border border-border rounded-3xl p-6 shadow-sm space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {paymentMode === "Credit" && (
                    <div className="hidden md:col-span-2 animate-in fade-in zoom-in duration-300">
                      <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wider block mb-2">
                ...
                <div className="hidden flex justify-between items-center p-3 bg-primary/10 rounded-xl border border-primary/20 mt-2">
                              ...
                              <div className="text-left w-[85%]">
                                ...
                                <div className="grid grid-cols-2 gap-y-1.5 gap-x-2">
                                  ...
                                  <span className="font-medium">
                                    {isMounted ? quotationNo : ""}
                                  </span>
                                  <span className="font-semibold text-slate-700">
                                    Date:
                                  </span>{" "}
                                  <span className="font-medium">
                                    {isMounted ? quotationDate : ""}
                                  </span>
                                  <span className="font-semibold text-slate-700">
                                    Due Date:
                                  </span>{" "}
                            ...
                            <tbody className="text-[11px] text-slate-800 align-top">
                              {items.length > 0 ? (
                                ...
                                <tr>
                                  ...
                                  >
                                    Add items to populate the quotation.
                                  </td>
                                </tr>
                              )}
                            </tbody>
    ...
    <span>{roundOffDiff > 0 ? '+' : ''}₹{roundOffDiff.toFixed(2)}</span>
                            ...
                            <div>
                              ...
                              <p className="text-slate-700 leading-relaxed whitespace-pre-line mt-2">
                                {notes ||
                                  "Thank you for your business!\nPlease make payment by the due date. Reference Quotation # on payment."}
                              </p>
                              {signatureUrl ? (
                            ...
                            <div className="w-[60%] flex gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-slate-600 text-[9px] mb-2">
                                  All Quotations are due with in 7 days from date
...
{enableRoundOff && roundOffDiff !== 0 && (
  <div className="flex justify-between py-1 text-[11px] font-medium">
    ...
    <span>{roundOffDiff > 0 ? '+' : ''}₹{roundOffDiff.toFixed(2)}</span>
                        ...
                        // TALLY PRIME CLASSIC GST QUOTATION LAYOUT (Strict grid, rounded-none, border-black)
                        <div
                        ...
                        >
                          ...
                          {/* Top Section: Company & Quotation Info */}
                          <div
